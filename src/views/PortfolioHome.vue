...
const projects = reactive([
  {
    id: 1,
    title: 'Momento郵寄遊記｜外國旅客台灣旅遊紀念服務',
...
.hero-spline { position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 1; overflow: hidden; }
.hero-spline iframe {
  position: absolute; top: 50%; left: 50%;
  width: 100vw; height: 100vh; min-width: 100%; min-height: 100%;
  transform: translate(-50%, -50%) scale(1.02);
  transform-origin: center center; border: none; display: block;
}
.hero-loading {
  position: absolute; inset: 0; z-index: 30; background: var(--color-bg);
...
.hero-overlay {
  ...
  padding: 0 64px 72px; pointer-events: none;
}
...
.card-placeholder { width:100%; height:100%; display:flex; align-items:center; justify-content:center; transition: transform 0.6s ease; overflow: hidden; }
...
.modal-placeholder { width:100%; height:240px; display:flex; align-items:center; justify-content:center; overflow: hidden; }
/* ★ Modal 圖片樣式 */
...
.modal-desc { font-size:15px; line-height:1.75; color: var(--color-text-secondary); margin-bottom:24px;white-space: pre-line; }
